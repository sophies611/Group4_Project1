...
Objective: For this project, we are using different regression models to predict whether the tele-marketing call will be successful or not. We created and improved each model to derive the best-performing model, as well as created five clusters to identify subsets. Subsequently, we ran each model on the clusters for evaluation.
...
teleann_model_2 <- neuralnet(formula = yyes ~ ., data = tele_train_2, hidden=c(2), stepmax=10^100, threshold=0.05)
...
teleann_model_3 <- neuralnet(formula = yyes ~ ., data = tele_train_3, hidden=c(1), threshold=0.05, stepmax=10^10, learningrate=0.01)
...
teleann_model_4 <- neuralnet(formula = yyes ~ ., data = tele_train_4, hidden=c(2,2), threshold=0.05, stepmax=10^100, learningrate=0.01)
...
teleann_model_5 <- neuralnet(formula = yyes ~ ., data = tele_train_5, hidden=c(3), threshold=0.05, stepmax=10^100)
